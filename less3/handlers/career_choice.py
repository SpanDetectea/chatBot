...
async def cmd_prof(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        f'Привет, {name}, '
        f'выбери профессию', reply_markup=make_row_keyboard(available_prof_names)
    )
...
async def cmd_chosen(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        f'Спасибо,тепреь выбери свой уровень ',
